...
#pragma message("AVX2")
        ...
        for (size_t i = 0; i < u256_count; ++i)
            _mm256_storeu_si256(u256ptr_dst + i,
                                _mm256_xor_si256(_mm256_lddqu_si256(u256ptr_dst + i),
                                                 _mm256_lddqu_si256(u256ptr_src + i))
            );

        if (u128_count)
            _mm_storeu_si128(u128ptr_dst,
                             _mm_xor_si128(_mm_loadu_si128(u128ptr_dst),
                                           _mm_loadu_si128(u128ptr_src))
            );

        if (u64_count)
            *u64ptr_dst ^= *u64ptr_src;
...
#pragma message("SSE2")
        ...
        for (size_t i = 0; i < u128_count; ++i)
            _mm_storeu_si128(u128ptr_dst + i,
                             _mm_xor_si128(_mm_loadu_si128(u128ptr_dst + i),
        ...
        if (u32_count)
            *u32ptr_dst ^= *u32ptr_src;

        if (u16_count)
            *u16ptr_dst ^= *u16ptr_src;
