...
const uint32_t accelc_SHA224_K[64] = {
...
void accelc_SHA224_init(SHA224_BUFFER* HashBuffer) {
    HashBuffer->dword[0] = 0xC1059ED8;
    HashBuffer->dword[1] = 0x367CD507;
    HashBuffer->dword[2] = 0x3070DD17;
    HashBuffer->dword[3] = 0xF70E5939;
    HashBuffer->dword[4] = 0xFFC00B31;
    HashBuffer->dword[5] = 0x68581511;
    HashBuffer->dword[6] = 0x64F98FA7;
    HashBuffer->dword[7] = 0xBEFA4FA4;
}
...
void accelc_SHA224_update(const void* srcBytes, size_t srcBytesLength, SHA224_BUFFER* HashBuffer) {
    ...
    for (size_t i = 0; i < RoundsOfMainCycle; ++i) {
        ...
        for (int j = 0; j < 64; ++j) {
            uint32_t T1 = h + (_rotr(e, 6) ^ _rotr(e, 11) ^ _rotr(e, 25)) + ((e & f) ^ (~e & g)) + accelc_SHA224_K[j] + Buffer[j];
...
void accelc_SHA224_final(const void* LeftBytes, size_t LeftBytesLength, uint64_t TotalBytesLength,
                         const SHA224_BUFFER* __restrict HashBuffer, SHA224_DIGEST* __restrict Hash) {
    SHA224_BUFFER temp_buf = *HashBuffer;

    if (LeftBytesLength >= SHA224_BLOCKSIZE) {
        accelc_SHA224_update(LeftBytes, LeftBytesLength, &temp_buf);
        LeftBytes = (const uint8_t*)LeftBytes + (LeftBytesLength / SHA224_BLOCKSIZE) * SHA224_BLOCKSIZE;
        LeftBytesLength %= SHA224_BLOCKSIZE;
    }
    ...
    *(uint64_t*)(Extra + (LeftBytesLength >= 64 - 8 ? 128 - 8 : 64 - 8)) = _byteswap_uint64(TotalBytesLength * 8);
    ...
    accelc_SHA224_update(Extra, LeftBytesLength >= 64 - 8 ? 128 : 64, &temp_buf);

    for (int i = 0; i < 7; ++i)
        Hash->dword[i] = _byteswap_ulong(temp_buf.dword[i]);
}

void accelc_SHA224(const void* srcBytes, size_t srclen, SHA224_DIGEST* Hash) {
    SHA224_BUFFER temp_buf;
    accelc_SHA224_init(&temp_buf);
    accelc_SHA224_update(srcBytes, srclen, &temp_buf);
    accelc_SHA224_final((uint8_t*)srcBytes + (srclen / SHA224_BLOCKSIZE) * SHA224_BLOCKSIZE, srclen % SHA224_BLOCKSIZE, srclen, &temp_buf, Hash);
}
