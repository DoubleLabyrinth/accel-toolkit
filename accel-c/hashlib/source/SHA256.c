...
#define HASH_SHA256_BLOCKSIZE 64

const uint32_t HASH_SHA256_K[64] = {
...
void HASH_SHA256_Initialize(uint32_t HashBuffer[8]) {
    HashBuffer[0] = 0x6A09E667;
    HashBuffer[1] = 0xBB67AE85;
    HashBuffer[2] = 0x3C6EF372;
    HashBuffer[3] = 0xA54FF53A;
    HashBuffer[4] = 0x510E527F;
    HashBuffer[5] = 0x9B05688C;
    HashBuffer[6] = 0x1F83D9AB;
    HashBuffer[7] = 0x5BE0CD19;
}
...
void HASH_SHA256_MainCycle(const void* srcBytes, size_t srcBytesLength, uint32_t HashBuffer[8]) {
    ...
    for (size_t i = 0; i < RoundsOfMainCycle; ++i) {
        ...
        for (int j = 0; j < 64; j++) {
            uint32_t T1 = h + (_rotr(e, 6) ^ _rotr(e, 11) ^ _rotr(e, 25)) + ((e & f) ^ (~e & g)) + HASH_SHA256_K[j] + Buffer[j];
...
void HASH_SHA256_Final(const void* LeftBytes, size_t LeftBytesLength, uint64_t TotalBytesLength, uint32_t HashBuffer[8]) {
    if (LeftBytesLength >= HASH_SHA256_BLOCKSIZE) {
        HASH_SHA256_MainCycle(LeftBytes, LeftBytesLength, HashBuffer);
        LeftBytes = (const uint8_t*)LeftBytes + (LeftBytesLength / HASH_SHA256_BLOCKSIZE) * HASH_SHA256_BLOCKSIZE;
        LeftBytesLength %= HASH_SHA256_BLOCKSIZE;
    }
    ...
    *(uint64_t*)(Extra + (LeftBytesLength >= 64 - 8 ? 128 - 8 : 64 - 8)) = _byteswap_uint64(TotalBytesLength * 8);
    ...
    HASH_SHA256_MainCycle(Extra, LeftBytesLength >= 56 ? 128 : 64, HashBuffer);
