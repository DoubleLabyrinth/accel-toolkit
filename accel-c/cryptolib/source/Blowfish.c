...
const uint32_t CRYPTO_Blowfish_orgPBox[18] = {
    0x243F6A88, 0x85A308D3, 0x13198A2E, 0x03707344, 0xA4093822, 0x299F31D0,
    0x082EFA98, 0xEC4E6C89, 0x452821E6, 0x38D01377, 0xBE5466CF, 0x34E90C6C,
    0xC0AC29B7, 0xC97C50DD, 0x3F84D5B5, 0xB5470917, 0x9216D5D9, 0x8979FB1B
};

const uint32_t CRYPTO_Blowfish_orgSBox[4][256] = {
    {
...
void CRYPTO_Blowfish_EncryptBlock(uint8_t srcBytes[8],
    ...
    if (Endian == CRYPTO_BLOWFISH_BIG_ENDIAN) {
        *L = _byteswap_ulong(*L);
        *R = _byteswap_ulong(*R);
    }

    *L ^= srcSubKeys[0];
    *R ^= f_transform(srcBytes, srcSBox);

    *R ^= srcSubKeys[1];
    *L ^= f_transform(srcBytes + 4, srcSBox);
...
int CRYPTO_Blowfish_KeyExpansion(uint8_t srcKey[], uint8_t srcKeyLength,
    ...
    if (srcKeyLength == 0)
        return CRYPTO_BLOWFISH_INVALID_KEY_LENGTH;
    if (srcKeyLength > CRYPTO_BLOWFISH_MAX_KEY_LENGTH)
        return CRYPTO_BLOWFISH_KEY_TOO_LONG;

    memcpy(dstSubKeys, CRYPTO_Blowfish_orgPBox, sizeof(CRYPTO_Blowfish_orgPBox));
    memcpy(dstSBox, CRYPTO_Blowfish_orgSBox, sizeof(CRYPTO_Blowfish_orgSBox));

    for (int i = 0; i < 18; ++i) {
        uint32_t temp = 0;

        temp <<= 8;
        temp |= srcKey[(i * 4) % srcKeyLength];

        temp <<= 8;
        temp |= srcKey[(i * 4 + 1) % srcKeyLength];

        temp <<= 8;
        temp |= srcKey[(i * 4 + 2) % srcKeyLength];

        temp <<= 8;
        temp |= srcKey[(i * 4 + 3) % srcKeyLength];
    ...
    uint8_t temp[8] = { 0 };
    for (int i = 0; i < 9; ++i) {
        CRYPTO_Blowfish_EncryptBlock(temp, dstSubKeys, dstSBox, 0);
        ((uint64_t*)dstSubKeys)[i] = *(uint64_t*)temp;
    }

    for (int i = 0; i < 512; ++i) {
        CRYPTO_Blowfish_EncryptBlock(temp, dstSubKeys, dstSBox, 0);
        ((uint64_t*)dstSBox)[i] = *(uint64_t*)temp;
    }

    return CRYPTO_BLOWFISH_SUCCESS;
}
