...
#define Swap(X, Y, Temp) Temp = X;  \
    ...
    ((uint64_t*)srcBytes)[0] ^= ((const uint64_t*)srcExpandedKey)[20];
    ((uint64_t*)srcBytes)[1] ^= ((const uint64_t*)srcExpandedKey)[21];
}
...
void CRYPTO_AES128_KeyExpansion(const uint8_t srcKey[16], uint32_t dstExpandedKey[44]) {
    ...
    for (int i = 4; i < 44; ++i) {
        ...
        if (i % 4 == 0) {
            ...
            temp ^= CRYPTO_AES_Rcon[i / 4];
        }
        dstExpandedKey[i] = dstExpandedKey[i - 4] ^ temp;
    }
}
