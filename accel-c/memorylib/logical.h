...
extern "C" {
    ...
    inline void MEMORY_XOR(void* dst, const void *src, size_t size) {
...
#error "Todo feature"
        ...
        for (size_t i = 0; i < round; ++i) 
            _mm256_storeu_si256(p_m256i_dst,
                                _mm256_xor_si256(_mm256_lddqu_si256(p_m256i_dst), 
                                                 _mm256_lddqu_si256(p_m256i_src))
            );
        ...
        for (size_t i = 0; i < round; ++i)
            _mm_storeu_si128(p_m128i_dst,
                             _mm_xor_si128(_mm_loadu_si128(p_m128i_dst),
                                           _mm_loadu_si128(p_m128i_src))
            );
        ...
        uint64_t* p_u64_dst = dst;
        const uint64_t* p_u64_src* = src;
    ...
    inline void MEMORY_AND(void* dst, const void *src, size_t size) {
#if defined(__AVX512F__)
#error "Todo feature"
#elif defined(__AVX2__)  // if AVX2 enabled
        const size_t round = size / (256 / 8);
        ...
        for (size_t i = 0; i < round; ++i)
            _mm256_storeu_si256(p_m256i_dst,
                                _mm256_and_si256(_mm256_lddqu_si256(p_m256i_dst),
                                                 _mm256_lddqu_si256(p_m256i_src))
            );
        ...
        for (size_t i = 0; i < round; ++i)
            _mm_storeu_si128(p_m128i_dst,
                             _mm_and_si128(_mm_loadu_si128(p_m128i_dst),
                                           _mm_loadu_si128(p_m128i_src))
            );
    ...
    inline void MEMORY_OR(void* dst, const void *src, size_t size) {
#if defined(__AVX512F__)
#error "Todo feature"
#elif defined(__AVX2__)  // if AVX2 enabled
        const size_t round = size / (256 / 8);
        ...
        for (size_t i = 0; i < round; ++i)
            _mm256_storeu_si256(p_m256i_dst,
                                _mm256_or_si256(_mm256_lddqu_si256(p_m256i_dst),
                                                _mm256_lddqu_si256(p_m256i_src))
            );
        ...
        for (size_t i = 0; i < round; ++i)
            _mm_storeu_si128(p_m128i_dst,
                             _mm_or_si128(_mm_loadu_si128(p_m128i_dst),
                                          _mm_loadu_si128(p_m128i_src))
            );
    ...
    inline void MEMORY_NOT(void* dst, size_t size) {
#if defined(__AVX512F__)
#error "Todo feature"
#elif defined(__AVX2__)  // if AVX2 enabled
        const size_t round = size / (256 / 8);
        ...
        for (size_t i = 0; i < round; ++i)
            _mm256_storeu_si256(p_m256i_dst,
                                _mm256_andnot_si256(_mm256_lddqu_si256(p_m256i_dst),
        ...
        for (size_t i = 0; i < round; ++i)
            _mm_storeu_si128(p_m128i_dst,
                             _mm_andnot_si128(_mm_loadu_si128(p_m128i_dst),
                                              _mm_setzero_si128())
            );
