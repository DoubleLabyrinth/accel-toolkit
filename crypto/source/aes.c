...
static const uint32_t _AES_Rcon[11] = {
        0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
};

static const uint8_t _GF2p8_MultiTable_0x02[256] = {
...
static const uint8_t _GF2p8_MultiTable_0x03[256] = {
...
static const uint8_t _AES_InverseSBox[256] = {
...
static const uint8_t _GF2p8_MultiTable_0x09[256] = {
...
static const uint8_t _GF2p8_MultiTable_0x0B[256] = {
...
static const uint8_t _GF2p8_MultiTable_0x0D[256] = {
...
static const uint8_t _GF2p8_MultiTable_0x0E[256] = {
...
void accelc_AES128_encrypt(uint8_t srcBytes[AES_BLOCK_SIZE], const AES128_KEY* srcKey) {
    ...
    for (int i = 1; i < 10; ++i) {
        ...
        for (int j = 0; j < 16; ++j)
            srcBytes[j] = _AES_SBox[srcBytes[j]];
        ...
        for (int j = 0; j < 16; j += 4) {
            ...
            *(uint32_t*)temp = ((uint32_t*)srcBytes)[j / 4];
            ...
            srcBytes[j] = (uint8_t)(_GF2p8_MultiTable_0x02[temp[0]] ^ _GF2p8_MultiTable_0x03[temp[1]] ^ temp[2] ^ temp[3]);
            srcBytes[j + 1] = (uint8_t)(temp[0] ^ _GF2p8_MultiTable_0x02[temp[1]] ^ _GF2p8_MultiTable_0x03[temp[2]] ^ temp[3]);
            srcBytes[j + 2] = (uint8_t)(temp[0] ^ temp[1] ^ _GF2p8_MultiTable_0x02[temp[2]] ^ _GF2p8_MultiTable_0x03[temp[3]]);
            srcBytes[j + 3] = (uint8_t)(_GF2p8_MultiTable_0x03[temp[0]] ^ temp[1] ^ temp[2] ^ _GF2p8_MultiTable_0x02[temp[3]]);
        }
...
void accelc_AES128_decrypt(uint8_t srcBytes[AES_BLOCK_SIZE], const AES128_KEY* srcKey) {
    ...
    for (int i = 9; i > 0; --i) {
        ...
        for (int j = 0; j < 16; ++j)
            srcBytes[j] = _AES_InverseSBox[srcBytes[j]];
        ...
        for (int j = 0; j < 16; j += 4) {
            uint8_t temp[4];
            *(uint32_t*)temp = ((uint32_t*)srcBytes)[j / 4];
            srcBytes[j] = (uint8_t)(_GF2p8_MultiTable_0x0E[temp[0]] ^ _GF2p8_MultiTable_0x0B[temp[1]] ^ _GF2p8_MultiTable_0x0D[temp[2]] ^ _GF2p8_MultiTable_0x09[temp[3]]);
            srcBytes[j + 1] = (uint8_t)(_GF2p8_MultiTable_0x09[temp[0]] ^ _GF2p8_MultiTable_0x0E[temp[1]] ^ _GF2p8_MultiTable_0x0B[temp[2]] ^ _GF2p8_MultiTable_0x0D[temp[3]]);
            srcBytes[j + 2] = (uint8_t)(_GF2p8_MultiTable_0x0D[temp[0]] ^ _GF2p8_MultiTable_0x09[temp[1]] ^ _GF2p8_MultiTable_0x0E[temp[2]] ^ _GF2p8_MultiTable_0x0B[temp[3]]);
            srcBytes[j + 3] = (uint8_t)(_GF2p8_MultiTable_0x0B[temp[0]] ^ _GF2p8_MultiTable_0x0D[temp[1]] ^ _GF2p8_MultiTable_0x09[temp[2]] ^ _GF2p8_MultiTable_0x0E[temp[3]]);
        }
    }
...
void accelc_AES128_set_key(const uint8_t srcUserKey[16], AES128_KEY* dstKey) {
    ...
    for (int i = 4; i < 44; ++i) {
        ...
        if (i % 4 == 0) {
            temp = _rotr(temp, 8);
            ((uint8_t*)&temp)[0] = _AES_SBox[((uint8_t*)&temp)[0]];
            ((uint8_t*)&temp)[1] = _AES_SBox[((uint8_t*)&temp)[1]];
            ((uint8_t*)&temp)[2] = _AES_SBox[((uint8_t*)&temp)[2]];
            ((uint8_t*)&temp)[3] = _AES_SBox[((uint8_t*)&temp)[3]];
            temp ^= _AES_Rcon[i / 4];
        }
