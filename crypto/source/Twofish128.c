/*********************************************************************
* Filename:   Twofish128.c
* Author:     Aiyu Xiao (xiao_ai_yu@live.cn)
*********************************************************************/
#include "../Twofish128.h"
#include <intrin.h>
...
void CRYPTO_Twofish128_EncryptBlock(uint8_t srcBytes[16], const uint32_t srcExpandedKey[40], const uint32_t srcS[2]) {
    ...
    for (int Round = 0; Round < 15; ++Round) {
        uint32_t T0 = func_h_128(srcBytes_dword_ptr[0], srcS);
        uint32_t T1 = _rotl(srcBytes_dword_ptr[1], 8);
        T1 = func_h_128(T1, srcS);
...
void CRYPTO_Twofish128_DecryptBlock(uint8_t srcBytes[16], const uint32_t srcExpandedKey[40], const uint32_t srcS[2]) {
    ...
    for (int Round = 14; Round >= 0; --Round) {
        T0 = func_h_128(srcBytes_dword_ptr[2], srcS);
        T1 = func_h_128(_rotl(srcBytes_dword_ptr[3], 8), srcS);
...
void CRYPTO_Twofish128_KeyExpansion(const uint8_t srcKey[16], uint32_t dstExpandedKey[40], uint32_t dstS[2]) {
    ...
    for (int i = 4; i < 8; ++i) {
        for (int j = 0; j < 8; ++j)
            ((uint8_t*)dstS)[i] ^= CRYPTO_Twofish_gf_0x14D_multi_table[CRYPTO_Twofish_RS[i - 4][j]][srcKey[j]];
    }
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 8; ++j)
            ((uint8_t*)dstS)[i] ^= CRYPTO_Twofish_gf_0x14D_multi_table[CRYPTO_Twofish_RS[i][j]][srcKey[8 + j]];
    }
