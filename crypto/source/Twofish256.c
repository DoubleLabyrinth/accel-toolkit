/*********************************************************************
* Filename:   Twofish256.c
* Author:     Aiyu Xiao (xiao_ai_yu@live.cn)
*********************************************************************/
#include "../Twofish256.h"
#include <intrin.h>
...
static uint32_t func_h_256(uint32_t X, const uint32_t L[4]) {
    ((uint8_t*)&X)[0] = CRYPTO_Twofish_q1[((uint8_t*)&X)[0]] ^ ((const uint8_t*)L)[12];
    ((uint8_t*)&X)[1] = CRYPTO_Twofish_q0[((uint8_t*)&X)[1]] ^ ((const uint8_t*)L)[13];
    ((uint8_t*)&X)[2] = CRYPTO_Twofish_q0[((uint8_t*)&X)[2]] ^ ((const uint8_t*)L)[14];
    ((uint8_t*)&X)[3] = CRYPTO_Twofish_q1[((uint8_t*)&X)[3]] ^ ((const uint8_t*)L)[15];
...
void CRYPTO_Twofish256_EncryptBlock(uint8_t srcBytes[16], const uint32_t srcExpandedKey[40], const uint32_t srcS[4]) {
    ...
    for (int Round = 0; Round < 15; ++Round) {
        uint32_t T0 = func_h_256(srcBytes_dword_ptr[0], srcS);
        uint32_t T1 = _rotl(srcBytes_dword_ptr[1], 8);
        T1 = func_h_256(T1, srcS);
...
void CRYPTO_Twofish256_DecryptBlock(uint8_t srcBytes[16], const uint32_t srcExpandedKey[40], const uint32_t srcS[4]) {
    ...
    for (int Round = 14; Round >= 0; --Round) {
        T0 = func_h_256(srcBytes_dword_ptr[2], srcS);
        T1 = func_h_256(_rotl(srcBytes_dword_ptr[3], 8), srcS);
...
void CRYPTO_Twofish256_KeyExpansion(const uint8_t srcKey[32], uint32_t dstExpandedKey[40], uint32_t dstS[4]) {
    ...
    for (int i = 12; i < 16; ++i) {
        for (int j = 0; j < 8; ++j)
            ((uint8_t*)dstS)[i] ^= CRYPTO_Twofish_gf_0x14D_multi_table[CRYPTO_Twofish_RS[i - 12][j]][srcKey[j]];
    }
    for (int i = 8; i < 12; ++i) {
        for (int j = 0; j < 8; ++j)
            ((uint8_t*)dstS)[i] ^= CRYPTO_Twofish_gf_0x14D_multi_table[CRYPTO_Twofish_RS[i - 8][j]][srcKey[8 + j]];
    }
    for (int i = 4; i < 8; ++i) {
        for (int j = 0; j < 8; ++j)
            ((uint8_t*)dstS)[i] ^= CRYPTO_Twofish_gf_0x14D_multi_table[CRYPTO_Twofish_RS[i - 4][j]][srcKey[16 + j]];
    }
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 8; ++j)
            ((uint8_t*)dstS)[i] ^= CRYPTO_Twofish_gf_0x14D_multi_table[CRYPTO_Twofish_RS[i][j]][srcKey[24 + j]];
    }
